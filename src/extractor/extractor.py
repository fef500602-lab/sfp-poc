...
IGNORE_CLASS_NAME_SUFFIXES = {
    ...
    "csharp":     [
        ...
        "service", "serviceimpl",       # camada de serviço não é FD
        "controller", "controllerimpl", # controllers são EPs, não FDs
        # Infraestrutura de segurança e configuração (TodoBackend C# — v4.4)
...
IGNORE_METHOD_NAMES = {
    "java": {
        ...
        # (identificado ao processar repos TodoBackend — v4.4)
...
IGNORE_DECORATORS = {
    ...
    "kotlin": [
        # Infraestrutura Kotlin — não são EPs de negócio
        "serializable",     # kotlinx.serialization — geração de código
        "module",           # Koin/Hilt DI modules
        "singleton",        # Koin scope
        "test",             # JUnit
    ],
}
...
FILE_ROLE_PATTERNS = {
    ...
    # v4.2: GraphQL como camada de interface potencialmente paralela ao REST.
    # Quando REST + GraphQL coexistem, os resolvers GraphQL duplicam a fronteira.
    # Métodos com file_role=graphql vão à LLM para decidir se são fronteira única
    # ou redundância de um controller REST já contado.
    "graphql":    ["graphql", "datafetcher", "datafetchers"],
...
def infer_file_role(filepath):
    ...
    HIGH_PRIORITY_ROLES = [
        ...
        ("feature",        ["feature", "features"]),
        # v4.2: GraphQL como camada potencialmente paralela — checar ANTES de model
        # para evitar que datafetchers em pastas de domínio sejam classificados como model.
        ("graphql",        ["graphql", "datafetcher", "datafetchers"]),
            ...
            if (inner.type == "call_expression" and
                ...
                if (fn_node and fn_node.type == "identifier" and args_node):
                    ...
                    if method in HTTP_ROUTE_METHODS:
                        ...
                        # v4.2: Filtra rotas de infraestrutura (health-check, readiness)
                        INFRA_ROUTES = {"/", "/health", "/ready", "/healthz", "/ping"}
                        if path and path.startswith("/") and path not in INFRA_ROUTES:
...
# ─────────────────────────────────────────
def classify_hint(name, base_classes, decorators, file_role, lang, is_method=False, field_names=None):
    ...
    #    v4.3: Antes de auto-classificar, verifica se é join table.
    for base in bases_lower:
        for pattern in DATA_FUNCTION_BASE_CLASSES.get(lang, []):
            if pattern in base:
                if field_names is not None and _is_join_table_candidate(field_names):
                    return "llm", (
                        f"herança '{base}' sugere FD, mas campos "
                        f"{field_names} indicam possível join table → LLM decide"
                    )
    ...
    for dec in decorators_lower:
        for pattern in DATA_FUNCTION_DECORATORS.get(lang, []):
            if pattern in dec:
                if field_names is not None and _is_join_table_candidate(field_names):
                    return "llm", (
                        f"decorator '{dec}' sugere FD, mas campos "
                        f"{field_names} indicam possível join table → LLM decide"
                    )
                return "data_function", f"decorator '{dec}' → padrão '{pattern}' em DATA_FUNCTION_DECORATORS[{lang}]"

    # 7b. file_role=graphql → canal potencialmente paralelo ao REST.
    #     v4.2: Repositórios com REST + GraphQL duplicam a fronteira funcional.
    #     Em vez de auto-classificar resolvers/datafetchers como EP (via decorator
    #     @DgsQuery/@DgsMutation), enviamos à LLM com contexto para que decida
    #     se é a fronteira única (sem REST correspondente → EP) ou redundante
    #     (REST controller já existe → ignore).
    if file_role == "graphql":
        if is_method:
            return "llm", "file_role=graphql + método → ambíguo (pode ser canal paralelo a REST)"
        return "ignore", "file_role=graphql + classe → ignorado (resolver/datafetcher não é FD)"
    ...
    if file_role == "controller":
        ...
        if lang == "java" and not decorators_lower:
            return "llm", "file_role=controller + método Java sem decorator → ambíguo (possível helper interno)"
        # v4.2: Em Kotlin/Ktor, rotas são declaradas funcionalmente:
        #   get("/articles") { ... }  → capturado por extract_ktor_routes
        # Métodos sem decorator em controller Kotlin (ex: fun Route.article())
        # são containers de routing, não operações — os EPs reais já foram
        # detectados como rotas Ktor. Ignorar para evitar dupla-contagem.
        if lang == "kotlin" and not decorators_lower:
            return "ignore", "file_role=controller + método Kotlin sem decorator → container de routing Ktor (EPs são as rotas funcionais)"
...
# ─────────────────────────────────────────
# 5b. Extração de campos/propriedades de classes
#
# v4.3: Usado para detecção de join tables.
# Classes com apenas campos FK (nomes terminando em Id/_id)
# não são Funções de Dados independentes — são tabelas associativas.
# ─────────────────────────────────────────
def extract_field_names(class_node, lang_name):
    """
    Extrai nomes de campos/propriedades de uma classe.
    Retorna lista de strings (nomes dos campos).
    """
    names = []

    if lang_name in ("java", "kotlin"):
        _collect_fields_jvm(class_node, names)
    elif lang_name == "csharp":
        _collect_fields_csharp(class_node, names)
    elif lang_name == "python":
        _collect_fields_python(class_node, names)
    elif lang_name in ("typescript", "tsx", "javascript"):
        _collect_fields_ts(class_node, names)

    return names


def _collect_fields_jvm(class_node, names):
    """Java/Kotlin: field_declaration, property_declaration, class_parameter."""
    for child in class_node.children:
        if child.type == "class_body":
            for member in child.children:
                if member.type == "field_declaration":
                    # Java: field_declaration → variable_declarator → identifier
                    for fc in member.children:
                        if fc.type == "variable_declarator":
                            for vc in fc.children:
                                if vc.type == "identifier":
                                    name = vc.text.decode("utf-8", errors="ignore")
                                    if name and not name.isupper():  # ignora constantes
                                        names.append(name)
                                    break
                elif member.type == "property_declaration":
                    # Kotlin: property_declaration → simple_identifier
                    for fc in member.children:
                        if fc.type in ("simple_identifier", "identifier"):
                            name = fc.text.decode("utf-8", errors="ignore")
                        ...
                        if fc.type == "variable_declaration":
                            for vc in fc.children:
                                if vc.type == "simple_identifier":
                                    names.append(vc.text.decode("utf-8", errors="ignore"))
                                    break
        elif child.type == "primary_constructor":
            # Kotlin data class: primary_constructor → class_parameter
            for param in child.children:
                if param.type == "class_parameter":
                    for pc in param.children:
                        if pc.type == "simple_identifier":
                            names.append(pc.text.decode("utf-8", errors="ignore"))
                            break


def _collect_fields_csharp(class_node, names):
    """C#: property_declaration e field_declaration em declaration_list."""
    for child in class_node.children:
        if child.type == "declaration_list":
            for member in child.children:
                if member.type == "property_declaration":
                    # property_declaration: type identifier accessor_list
                    for fc in member.children:
                        if fc.type == "identifier":
                            names.append(fc.text.decode("utf-8", errors="ignore"))
                            break
                                ...
                                if vc.type == "variable_declarator":
                                    for vd in vc.children:
                                        if vd.type == "identifier":
                                            names.append(vd.text.decode("utf-8", errors="ignore"))
                                            break


def _collect_fields_python(class_node, names):
    """Python: class-level assignments (Django model fields: title = models.CharField(...))."""
    for child in class_node.children:
        if child.type == "block":
                        ...
                        if expr.type == "assignment":
                            for ac in expr.children:
                                if ac.type == "identifier":
                                    name = ac.text.decode("utf-8", errors="ignore")
                                    # Ignora campos privados, constantes, e Meta
...
def _collect_fields_ts(class_node, names):
    """TypeScript/JS: property_signature, public_field_definition."""
    for child in class_node.children:
...
# ─────────────────────────────────────────
# 5c. Detecção de join tables
#
# v4.3: Heurística para identificar tabelas associativas many-to-many
# que NÃO devem ser contadas como Funções de Dados independentes.
# ─────────────────────────────────────────
# Campos de auditoria/infraestrutura — não contam como "de negócio"
_AUDIT_FIELDS = {
    "id", "createdat", "updatedat", "created_at", "updated_at",
    "createdby", "updatedby", "created_by", "updated_by",
    "version", "rowversion", "timestamp",
}

# Sufixos que indicam Foreign Key
_FK_SUFFIXES = ("id", "_id", "key", "_key", "ref", "_ref")


def _is_join_table_candidate(field_names):
    """
    Heurística: classe é candidata a join table se, após remover campos
    de auditoria/ID, todos os campos restantes parecem ser Foreign Keys.

    Ex: ArticleFavorite(id, articleId, userId, createdAt) → join table
    Ex: Article(id, title, body, slug, authorId, createdAt) → entidade real
    """
    if not field_names:
        return False  # Sem campos extraídos → não temos info suficiente

    # Separa campos de auditoria dos campos "meaningful"
    meaningful = []
    for name in field_names:
        if name.lower() in _AUDIT_FIELDS:
            continue
        meaningful.append(name.lower())

    if not meaningful:
        return False  # Só tem campos de auditoria → provavelmente base class

    if len(meaningful) > 3:
        return False  # Muitos campos → provavelmente entidade real

    # Verifica se TODOS os campos meaningful parecem FKs
    fk_count = sum(
        1 for name in meaningful
        if any(name.endswith(suffix) for suffix in _FK_SUFFIXES)
    )

    # Join table: pelo menos 2 FKs e NENHUM campo de negócio
    return fk_count >= 2 and fk_count == len(meaningful)
    ...
    def walk(node):
        ...
        if node.type in ("class_definition",          # Python
            ...
            if name:
                ...
                # v4.3: Extrai campos para detecção de join tables
                fields = extract_field_names(node, lang_name)

                hint, hint_reason = classify_hint(
                    name, bases, decs, file_role, lang_name,
                    is_method=False, field_names=fields,
                )
                ...
                item = {
                    ...
                    "base_classes": bases,
                    "decorators":   decs,
                    "fields":       fields,     # v4.3: incluído para auditoria
                    "sfp_hint":     hint,
...
def analyze_repository(repo_path, repo_name):
    ...
    # v4.3: Pós-processamento com visão global do repositório
    _post_process_repository(report)

    return report


# ─────────────────────────────────────────
# 8b. Pós-processamento global
#
# v4.3: Detecta padrões que classify_hint (por arquivo) não pode ver:
#   1. Duplicação cross-canal: REST + GraphQL para mesma operação
#   2. Validação cruzada de FDs com campos (auditoria)
#
# Roda DEPOIS de coletar todos os itens do repositório, com visão
# global de todos os EPs e FDs extraídos.
# ─────────────────────────────────────────
def _post_process_repository(report):
    """Pós-processamento com visão global do repositório."""
    all_eps = report["elementary_processes"]

    # --- 1. Cross-channel dedup: REST + GraphQL ---
    # Se o mesmo EP (por nome) aparece em controller (REST) e graphql,
    # a versão GraphQL é canal paralelo — remover.
    controller_names = set()
    for ep in all_eps:
        if ep.get("file_role") == "controller" and ep.get("sfp_hint") == "elementary_process":
            controller_names.add(ep["name"].lower())

    if controller_names:
        cleaned_eps = []
        ignored     = []
        for ep in all_eps:
            if (ep.get("file_role") == "graphql"
                    and ep["name"].lower() in controller_names):
                ep["sfp_hint"]    = "ignore"
                ep["hint_reason"] = (
                    f"post-processor: EP '{ep['name']}' duplicado — "
                    f"mesmo nome existe em controller REST → canal paralelo ignorado"
                )
                ignored.append(ep)
            else:
        ...
        if ignored:
            report["elementary_processes"] = cleaned_eps
            report.setdefault("post_processor_ignored", []).extend(ignored)
            print(f"   🔄 Post-processor: {len(ignored)} EPs duplicados (GraphQL ∩ REST) removidos")

    # --- 2. Log de FDs que passaram pela detecção de join table ---
    join_candidates = [
        fd for fd in report["data_functions"]
        if "join table" in fd.get("hint_reason", "").lower()
    ]
    if join_candidates:
        names = [fd["name"] for fd in join_candidates]
        print(f"   🔗 Post-processor: {len(join_candidates)} FDs candidatas a join table → LLM: {names}")
