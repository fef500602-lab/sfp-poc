...
- CANAIS PARALELOS (v4.2): se o sistema expõe a mesma operação de negócio
  por REST E GraphQL (ou gRPC, WebSocket), contar apenas UMA fronteira.
  Exemplos: se ArticlesApi.createArticle (REST) e ArticleMutation.createArticle
  (GraphQL) existem, classifique o controller REST como EP e o resolver GraphQL
  como "ignore" — representam a mesma operação.
- TABELAS DE JUNÇÃO (v4.2): tabelas associativas many-to-many sem atributos
  de negócio próprios (ex: ArticleFavorite com apenas article_id + user_id)
  NÃO são Funções de Dados independentes — são parte do relacionamento.
  Somente classifique como FD se a tabela tiver atributos com significado
  de negócio além das chaves estrangeiras (ex: createdAt conta se for
  informação de negócio, não se for apenas auditoria técnica).
- CONTAINERS DE ROUTING: funções que apenas agrupam/configuram rotas
  (ex: fun Route.article() { ... } em Ktor) NÃO são EPs — os EPs são
  as rotas individuais dentro delas. Classificar como "ignore".
